...
def Q_learning(num_episodes=10000, gamma=0.9, epsilon=1, decay_rate=0.999):
	...
	for iteration in range(0, num_episodes):
		...
		current_Observation,_,_,_ = env.reset()
		total_reward = 0
		if(iteration % 1000 == 0):
			print("Iteration: ", iteration)
		while True:

			#Add in a new row if we have not encountered this state before
			hashed_state = hash(current_Observation)
			if hashed_state not in Q_table:
				Q_table[hashed_state] = np.zeros(env.action_space.n)
				updateNumber_Table[hashed_state] = np.zeros(env.action_space.n)


			#If the current epsilon has decayed enough(with some randomess), choose the max to settle to optimize rather than random choices for data
			if np.random.rand() > cur_epsilon:
				action = np.argmax(Q_table[hashed_state])
			else:
				action = env.action_space.sample()

			# Get the new obervation space from the action
			new_obs, new_reward, in_completion_state, _ = env.step(action)
			total_reward += new_reward

			#Turns out python supports unicode, so we can be fancy with the variables naming!
			#Anyways the new update number is just one more data point found
			η = 1 / (1 + updateNumber_Table[hashed_state][action])

			#If this state hasn't been explored, add it to the table
			hashed_obs = hash(new_obs)
			if hashed_obs not in Q_table:
				Q_table[hashed_obs] = np.zeros(env.action_space.n)
				updateNumber_Table[hashed_obs] = np.zeros(env.action_space.n)


			#Define the reward from the old state, with the new action and its observation
			v = np.max(Q_table[hashed_obs])
			Q_table[hashed_state][action] = (1 - η) * Q_table[hashed_state][action] + η * (new_reward + gamma * v)

			# increment count the count by 1
...
def conduct_evaluations():
	...
	# creating a map for each action taken when in heal state or when with a guard
	action_map = {
		"HEAL": np.zeros(env.action_space.n),
	...
	for _ in tqdm(range(EVAL_EPISODE_COUNT)):
		...
		while not done:
			...
			# update action map
			if obs.get("at_heal"):
				action_map["HEAL"][action] += 1
			if obs.get('guard_in_cell'):
				guard = obs.get('guard_in_cell')
				if guard in action_map:
					action_map[guard][action] += 1
	...
	end_time = time.time()
	print("Number of unique states from Q table", len(Q_table))
	print("The average reward over 10,000 evaluation episodes:", avg_reward)
	print("The average length of an episode (number of actions taken) during evaluation:", total_steps / EVAL_EPISODE_COUNT)
	print("The total (user) time taken to play 10000 episodes using the saved Q-table.", end_time - start_time, " seconds.")
	print("Number of unique states encountered during evaluation: ", len(new_states))
	print("Percentage of actions from Q-table: ", (actions / total_actions) * 100)
	print("Percentage of random actions taken due to missing Q-values: ", (random_actions / total_actions) * 100)



	#To be frank I did not know pyplot, so I consulted with Jessica Hagihara on how to set it up
	# I understand it now, but the setup for the final graph will probably be very similiar.
	for key in action_map:
		total = np.sum(action_map[key])
		if total > 0:
			action_map[key] = action_map[key] / total

	rows = list(action_map.keys())
	counts = np.array([action_map[r] for r in rows])
	action_labels = ["UP", "DOWN", "LEFT", "RIGHT", "FIGHT", "HIDE", "HEAL", "WAIT"]
	# plot heatmap
	plt.figure(figsize=(10, 6))
	plt.imshow(counts, cmap="coolwarm", aspect="auto")

	plt.xticks(range(len(action_labels)), action_labels)
	plt.yticks(range(len(rows)), rows)
	plt.colorbar(label="Action frequency")
	plt.title("Action distribution by state (heal or guards)")
	plt.xlabel("Action")
	plt.ylabel("State")
